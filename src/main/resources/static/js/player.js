...
function updateProgress(){
	...
	music_bar.goto(rem.audio[0].currentTime / rem.audio[0].duration);
     var  durations=formatTime(rem.audio[0].duration); //歌曲总时间
     //$(".music-alltime").html(durations);
     var curTime = formatTime(rem.audio[0].currentTime);//歌曲当前时间
     //$(".music-nowtime").html(curTime);
    $(".audio-time").html(curTime+"/"+durations);
    // 同步歌词显示
...
function play(music) {
    ...
    $(".music-nowname").html(music.name+" -- "+music.artist);
