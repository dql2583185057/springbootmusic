    ...
    $.ajax({
       ...
       // data: "types=search&count=" + mkPlayer.loadcount + "&source=" + rem.source + "&pages=" + rem.loadPage + "&name=" + rem.wd,
        ...
        success: function (jsonData) {
            ...
            var data = eval('(' + jsonData.msg + ')');
            console.log(data.result.songs);
            // 调试信息输出
            if (mkPlayer.debug) {
                console.debug("搜索结果数：" + data.length);
            }

            if (rem.loadPage == 1)   // 加载第一页，清空列表
            {
                if (data.length === 0)   // 返回结果为零
                {
            ...
            for (var i = 0; i < data.result.songs.length; i++) {
                no++;
                tempItem = {
                    id: data.result.songs[i].id,  // 音乐ID
                    name: data.result.songs[i].name,  // 音乐名字
                    artist: data.result.songs[i].ar[0].name, // 艺术家名字
                    album: data.result.songs[i].al.name,    // 专辑名字
                    url_id: data.result.songs[i].id,  // 链接ID
                    pic_id: null,  // 封面ID
                    lyric_id: data.result.songs[i].id,  // 歌词ID
                    pic: data.result.songs[i].al.picUrl,    // 专辑图片
                    url: null   // mp3链接
                };
...
function ajaxUrl(music, callback) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            ...
            if (data.data[0].url === "") {
                data.data[0].url = "https://music.163.com/song/media/outer/url?id=" + music.id + ".mp3";
            } else {
                data.data[0].url = data.data[0].url.replace(/m7c.music./g, "m7.music.");
                data.data[0].url = data.data[0].url.replace(/m8c.music./g, "m8.music.");
            }
            ...
            if (data.data[0].url === "") {
                ...
                music.url = data.data[0].url;    // 记录结果
            }
...
function ajaxPlayList(lid, id, callback) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            console.log("歌单信息：");
            var data = JSON.parse(jsonData.msg);
            console.log(data);
            // 存储歌单信息
            var tempList = {
                id: lid,    // 列表的网易云 id
            ...
            if (data.playlist.coverImgUrl !== '') {
                tempList.cover = data.playlist.coverImgUrl + "?param=200y200";
            } else {
            ...
            if (typeof data.playlist.tracks !== undefined || data.playlist.tracks.length !== 0) {
                ...
                for (var i = 0; i < data.playlist.tracks.length; i++) {
                    tempList.item[i] = {
                        ...
                        pic: data.playlist.tracks[i].al.picUrl + "?param=300y300",    // 专辑图片
                        url: null   // mp3链接
                    };
                }
            }
...
function ajaxLyric(music, callback) {
    ...
    $.ajax({
        ...
        datatype: "json",
        success: function (jsonData) {
            var schema = eval('(' + jsonData.msg + ')');//解决多一对引号的问题
            // 调试信息输出
            if (mkPlayer.debug) {
                console.debug("歌词获取成功");
            }
            if (schema.lrc.lyric) {
                callback(schema.lrc.lyric, music.lyric_id);    // 回调函数
            } else {
...
function ajaxUserList(uid) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            var userdata = eval('(' + jsonData.msg + ')');//解决多一对引号的问题
            if (userdata.code == "-1" || userdata.code == 400) {
                layer.msg('用户 uid 输入有误');
                return false;
            }
            console.log(userdata);
            if (userdata.playlist.length === 0 || typeof (userdata.playlist.length) === "undefined") {
                ...
                rem.uname = userdata.playlist[0].creator.nickname;  // 第一个列表(喜欢列表)的创建者即用户昵称
                ...
                for (var i = 0; i < userdata.playlist.length; i++) {
                    // 获取歌单信息
                    tempList = {
                        id: userdata.playlist[i].id,    // 列表的网易云 id
                        name: userdata.playlist[i].name,   // 列表名字
                        cover: userdata.playlist[i].coverImgUrl + "?param=200y200",   // 列表封面
                        creatorID: uid,   // 列表创建者id
                        creatorName: userdata.playlist[i].creator.nickname,   // 列表创建者名字
                        creatorAvatar: userdata.playlist[i].creator.avatarUrl,   // 列表创建者头像
                        item: []
                    };
                    // 存储并显示播放列表
