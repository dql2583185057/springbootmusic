...
function comments(obj) {
    ...
    $.ajax({
        ...
        success: function (jsonData) {
            var schema = eval('(' + jsonData.msg + ')');

            if (schema.hotComments && schema.hotComments.length) {
                rem.comments = schema.hotComments;
            } else if (schema.comments && schema.comments.length) {
                rem.comments = schema.comments;
            } else {
                rem.comments = [];
                return;
            }
            var commentsIndex = 0;
            $(".banner_text .nikename").text(rem.comments[0].user.nickname+":");
            $(".banner_text .pinlun").text(rem.comments[0].content);
            ...
            //
            // }
            $(".banner_text img").show().attr("src", rem.comments[0].user.avatarUrl ? rem.comments[0].user.avatarUrl : "images/avatar.png");
            rem.commentsTime = setInterval(function () {
                ...
                $(".banner_text .nikename").text( rem.comments[commentsIndex].user.nickname+":");
                $(".banner_text .pinlun").text(rem.comments[commentsIndex].content);
                $(".banner_text img").show().attr("src", rem.comments[commentsIndex].user.avatarUrl ? rem.comments[commentsIndex].user.avatarUrl : "images/avatar.png");
            }, 5000)
        },   //success
